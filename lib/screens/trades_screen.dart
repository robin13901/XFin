...
import '../database/tables.dart'; // Import TradeWithAsset
...
class TradesScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final pnlFormat = NumberFormat.currency(locale: 'de_DE', symbol: '€', decimalDigits: 2);
    ...
    return Scaffold(
      ...
      body: StreamBuilder<List<TradeWithAsset>>(
        ...
        builder: (context, snapshot) {
          ...
          return ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              final datetime = DateTime.parse('${rawDatetime.substring(0, 8)} ${rawDatetime.substring(8, 14)}');

              List<Text> subtitleWidgets = [
                Text('${l10n.datetime}: ${dateTimeFormatter.format(datetime)} Uhr'),
                Text('${l10n.value}: ${currencyFormat.format(trade.portfolioAccountValueDelta.abs())}'),
                Text('${l10n.tradingFee}: ${currencyFormat.format(trade.tradingFee)}'),
                if (trade.type == TradeTypes.sell) Text('${l10n.tax}: ${currencyFormat.format(trade.tax)}'),
              ];

              if (trade.type == TradeTypes.sell) {
                final pnlColor = trade.profitAndLossAbs >= 0 ? Colors.green : Colors.red;
                  ...
                  Text.rich(
                    TextSpan(
                      children: [
                        ...
                        TextSpan(
                          text: pnlFormat.format(trade.profitAndLossAbs), // Value in color
                  ...
                  Text.rich(
                    TextSpan(
                      children: [
                        ...
                        TextSpan(
                          text: '${formatter.format(trade.profitAndLossRel * 100)} %', // Value in color
                          style: TextStyle(color: pnlColor),
                        ),
                      ],
                    ),
              ...
              return ListTile(
                title: Text(
                    '${trade.type.name.toUpperCase()} ${trade.shares} ${asset.tickerSymbol} @ ${currencyFormat.format(trade.pricePerShare)}'),
                subtitle: Column(
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () => _showTradeForm(context),
